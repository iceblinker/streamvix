...
function logToFile(msg: string) { console.log(`[DEBUG_VIX] ${msg}`); }
...
export async function getStreamContent(id: string, type: ContentType, config: ExtractorConfig): Promise<VixCloudStreamInfo[] | null> {
  logToFile(`getStreamContent called for ${id}`);
  /* console.log(...) replaced/augmented */

  // Resolve URL
  const url = await getUrl(id, type, config);
  logToFile(`Resolved URL: ${url}`);
  if (!url) return null;

  try {
    logToFile(`Fetching ${url}...`);
    ...
    if (!response.ok) {
      logToFile(`Failed to fetch page: ${response.status}`);
      return null;
    }
    const html = await response.text();
    logToFile(`Page HTML length: ${html.length}`);
    ...
    logToFile(`getStreamContent Error: ${(e as any).message}`);
    return null;
  }
}
